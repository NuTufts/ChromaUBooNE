...
__device__ bool 
intersect_box_axis_aligned( const int& aligned_axis, const float3 &origin, const float3 &direction, 
	      const float3 &lower_bound, const float3 &upper_bound,
	      float& distance_to_box)
{
		     ...
		     float& tminbox, float& tmaxbox)
{
  ...
  const float c0hix = upper_bound.x * idir.x - ood.x;
