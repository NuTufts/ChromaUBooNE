...
from npycacheable import NPYCacheable
...
class Mesh(NPYCacheable):
    "Triangle mesh object."
    def __init__(self, vertices=None, triangles=None, remove_duplicate_vertices=False):
        if vertices is None and triangles is None:
            log.debug("created zombie Mesh : needs populating from cache")
            return
...
class Geometry(NPYCacheable):
    ...
    def flatten(self):
        ...
        logger.debug('Flattening detector mesh...')
        logger.debug('  triangles: %d' % len(triangles))
        logger.debug('  vertices:  %d' % len(vertices))
